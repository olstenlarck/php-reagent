...
class User extends Controller
{
    ...
    {
        if (Session::getSession('logged') === false) {
            header('Location: index.php');
        } else {
            $this->loadModel('user');
            if (isset($_POST['changePassword'])) {
                $sessUserName = Session::getSession('userName');
    ...
    {
        if (Session::getSession('logged') === true) {
            header('Location: index.php');
        } else {
            $this->loadModel('user');
                    ...
                    if ($this->minLen($userName) !== false && $this->minLen($userPass) !== false) {
                        ...
                        if (mysql_num_rows($this->checkQuery) > 0) {
                            ...
                            while ($row = sqlFetchRow($this->checkQuery)) {
                                Session::setSession('userID', $row['userID']);
                                Session::setSession('userGroup', $row['userGroup']);
